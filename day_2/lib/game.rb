...
class  Game
  ...
  def new_players_in_sight
    if @enemies_in_sight.size > @players_left
      return puts "Tout les joueurs sont là"
    end
    key = rand(1..6)
    case key
    when (2..4)
      ...
      puts "Un nouvelle ennemis #{new_enemie.name} arrive"
    when 5,6
      ...
      puts "Deux nouveaux ennemis #{new_enemie1.name} & #{new_enemie2.name} arrive"
    else
      puts "Aucun nouvel ennemis arrive"
    end
  end
